...
export const useDebug = <Props extends Record<string, any> = any>(componentName: string, props: Props) => {
  const componentDisplayName = `<${componentName} />`

  const renderCount = useRenderCount()

  const changedProps = useRef<Record<string, { current: any; previous: any }>>({})
