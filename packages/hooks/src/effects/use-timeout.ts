 ...
 */
export function useTimeout2<T = void>(callback?: () => T, time?: number): IUseTimeoutReturn<T> {
  const timeout = useRef<NodeJS.Timeout | number | string | undefined>(undefined)

  const [waiting, setWaiting] = useState(false)

  const resolvePromise = useRef<(value: PromiseLike<T> | T) => void>(undefined)
