...
export function useCopy() {
  ...
  const copyToClipboard = (text: string, options: { onFailure?: () => void; onSuccess?: () => void } = {}) => {
    ...
    if (typeof navigator !== 'undefined' && navigator.clipboard) {
      ...
      console.warn('You need to use a secure context to use clipboard \n Please use the following link:', text)
      onFailure()
    }
  }
  ...
  const fetchAndCopyToClipboard = (
    ...
    options: { onFailure?: () => void; onSuccess?: () => void } = {}
  ) => {
    const { onSuccess = noop, onFailure = noop } = options
