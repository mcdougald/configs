...
import { useVirtual as useVirtualImpl, type VirtualItem } from 'react-virtual'
...
Options<T>): Virtualizer => {
  const measureRefCacheRef = useRef<Record<string, (el: HTMLElement | null) => void>>({})
  ...
  const virtualItems = rowVirtualizer.virtualItems.map((item) => {
    ...
    const measureRef = cachedMeasureRefWrappers[item.key] ?? (() => {})
