...
import { type Options as VirtualOptions, type Range, useVirtual } from 'react-virtual'
...
const extractBufferendRange = ({ start, end, overscan, size }: Range, current: BufferedRange): number[] => {
  const rangeStart =
...
export const useBufferedVirtual = <T>(options: VirtualOptions<T>): ReturnType<typeof useVirtual> => {
  const currentRangeRef = useRef<BufferedRange>(undefined)
  const rangeExtractor = useCallback((range: Range) => {
    return extractBufferendRange(range, currentRangeRef.current ?? { start: range.start, end: range.end })
  }, [])
  ...
  const virtualItemsEnd = result.virtualItems[lastVirtualIndex]?.index ?? lastVirtualIndex
