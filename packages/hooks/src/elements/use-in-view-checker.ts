...
export default <T extends HTMLElement>(ref: MutableRefObject<T>, margin = 1): [boolean, Dispatch<boolean>] => {
  const getIsInView = () => {
    if (!ref?.current) {
      return true
    }
  ...
  const checkIsInView = () => {
    ...
    timer.current = setTimeout(() => {
      ...
      animationFrame.current = globalThis.requestAnimationFrame(checkIsInView)
    })
  }
