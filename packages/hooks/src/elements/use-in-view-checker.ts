...
const isCompletelyInView = (el: HTMLElement, margin = 1) => {
	...
	return (
		el.contains(
			document.elementFromPoint(rect.left + margin, rect.top + margin),
		) &&
		el.contains(
			document.elementFromPoint(rect.left + margin, rect.bottom - margin),
		) &&
		el.contains(
			document.elementFromPoint(rect.right - margin, rect.top + margin),
		) &&
		el.contains(
			document.elementFromPoint(rect.right - margin, rect.bottom - margin),
		)
	);
};
