import type { DimensionObject } from './use-dimensions'
...
export type ElementAnchor = HorizontalAnchor | VerticalAnchor
...
export type UseRelativePositionHook = [
  ...
  (node: HTMLElement) => void,
  [AbsolutePosition, 'none' | RelativePosition]
]
...
const getScrollParent = (node: Element): Element | null =>
  !node || node === document.body ? document.body : scroll(node) ? node : getScrollParent(node.parentNode as Element)

/**
 *
 * Taken from Radix
 * @param root0
 * @param root0.allowedPositions
 * @param root0.spacing
 * @param deps
 * @returns {UseRelativePositionHook}
 */
export default function useRelativePosition(
  ...
  const [baseElementRef, baseElementDimensions, baseNode] = useDimensions({ liveMeasure: true }, deps)
  const [popinElementRef, popinElementDimensions, popinNode] = useDimensions({ liveMeasure: true }, [
    ...deps,
    baseElementDimensions
  ])
  const [state, setState] = useState<[AbsolutePosition, 'none' | RelativePosition]>([null, 'none'])
  ...
  useEffect(() => {
    ...
    if (
      !allowedPositions?.some((allowedPosition) => {
        const baseAnchorTranslation = computeAnchorTranslation(baseElementDimensions, allowedPosition.base)
        const popinAnchorTranslation = computeAnchorTranslation(popinElementDimensions, allowedPosition.popin)
        ...
        switch (allowedPosition.base.side) {
          ...
          case 'right': {
            horizontalPosition = {
              left: baseElementDimensions.width - popinAnchorTranslation.x + spacing,
              right: UNSET
            }
            verticalPosition = {
              top: baseAnchorTranslation.y - popinAnchorTranslation.y,
          ...
          case 'top': {
            ...
            verticalPosition = {
              top: UNSET,
              bottom: baseElementDimensions.height - popinElementDimensions.height + popinAnchorTranslation.y + spacing
            }
            break
          }
        }
        ...
        const baseRightSpace = (parentRect?.width || 0) - (baseLeftSpace + baseElementDimensions.width)
        const baseBottomSpace = (parentRect?.height || 0) - (baseTopSpace + baseElementDimensions.height)
        const leftSpace = baseLeftSpace + baseElementDimensions.width - baseAnchorTranslation.x
        const rightSpace = baseRightSpace + baseElementDimensions.width - baseAnchorTranslation.x
        const topSpace = baseTopSpace + baseElementDimensions.height - baseAnchorTranslation.y
        const bottomSpace = baseBottomSpace + baseElementDimensions.height - baseAnchorTranslation.y

        if (position.left === UNSET) {
          const hasEnoughSpaceOnTheLeft =
            popinElementDimensions.width - (popinElementDimensions.width - popinAnchorTranslation.x) <= leftSpace

          if (!hasEnoughSpaceOnTheLeft) {
            return false
          }
        } else {
          const hasEnoughSpaceOnTheRight = popinElementDimensions.width - popinAnchorTranslation.x <= rightSpace
        ...
        if (position.top === UNSET) {
          const hasEnoughSpaceOnTheTop =
            popinElementDimensions.height - (popinElementDimensions.height - popinAnchorTranslation.y) <= topSpace

          if (!hasEnoughSpaceOnTheTop) {
            return false
          }
        } else {
          const hasEnoughSpaceOnTheBottom = popinElementDimensions.height - popinAnchorTranslation.y <= bottomSpace
 ...
 * @param anchor
 */
export function computeAnchorPoint(dimensions: DimensionObject, anchor: ElementAnchor): Position {
 ...
 * @param anchor
 */
export function computeAnchorTranslation(dimensions: DimensionObject, anchor: ElementAnchor): Position {
  let x: number
  ...
  switch (anchor.side) {
    ...
    default: {
      throw new Error('E_INVALID_ANCHOR', anchor)
    }
  }
